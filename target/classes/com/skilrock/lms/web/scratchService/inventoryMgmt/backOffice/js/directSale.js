...
alert(i18nObj.prop('error.pls.remove.invalid.entries.first'));hidSubmit();
return false;
}}}}
else{alert(i18nObj.prop('error.pls.varify.entries.first'));hidSubmit();return false;}
if(_isS=="false"){
	alert(i18nObj.prop('error.bal.limit.not.enough.fr.direct.sale'));hidSubmit();return false;
}
...
function verifyScratchBoToRetailerBooks(){
	...
	var _resp  = _ajaxCall(url);
		...
		_vf=true;_id.vis('subTrans');
			if(_isS=="false"){hidSubmit();
			alert(i18nObj.prop('error.bal.limit.not.enough.fr.direct.sale'));
			}
			}else{hidSubmit();}
	_id.i('subCheck','');
	_id.vis('verBk');
...
if(gmNoLnth<1)
_id.i('top_form','<h3>'+document.getElementsByTagName('h3')[0].innerHTML+'</h3><br/><center>'+i18nObj.prop('msg.no.game.avail.fr.sale')+'</center>');
}
else{
gmNoLnth = numOfGame;
}
...
var gameSel = document.getElementsByName("gameName");
		...
		if(gameSel[i].selectedIndex!="0"){
			...
			alert(i18nObj.prop('error.u.have.already.selected.game.pls.use.addmore.btnIf.u.need.more.entries.fr.same.game'));
			gameSel[i].focus();
...
function validateData(){
		var chkD = checkDupBk();hidSubmit();
	  	...
	  	if(chkD.Emp){
		_id.i('retOrgDiv',_id.o('retOrgDiv').innerHTML+'<br/>'+i18nObj.prop('error.pls.fill.one.valid.entry'),'e');
		}
		alert(i18nObj.prop('error.pls.check.error.msg.on.page'));
		return false;
}
function frmValidtr(){
...
_id.i('retOrgDiv',i18nObj.prop('error.pls.slct.any.one.game'),'e');
gameSel[0].focus();
return false;
}else if (_id.o('retOrgDiv').innerHTML!=""&&isEmpty){
_id.i('retOrgDiv',_id.o('retOrgDiv').innerHTML+'<br/>'+i18nObj.prop('error.pls.slct.any.one.game'),'e');
return false;
}
return true;
}
...
_id.i('gamemessage'+gameIndex,i18nObj.prop('error.pls.slct.game.first'),'e');
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/inventoryMgmt/backOffice/js/directSale.js,v $'] = '$Id: directSale.js,v 1.3 2016/10/31 09:46:19 neeraj Exp $';
