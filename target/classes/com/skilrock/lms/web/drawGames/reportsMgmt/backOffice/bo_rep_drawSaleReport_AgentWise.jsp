...
<s:if test="#session.reportList.size>0">
				<s:div id="excel"
					...
					<s:a href="bo_rep_drawSaleReport_exportExcel.action?reportType=Agent Wise&start_date=%{#startDate}&end_Date=%{#endDate}&stateCode=%{#state}&cityCode=%{#city}"><s:text name="btn.exportasexcel"/></s:a>
				</s:div>
</s:if>
	...
	<s:if
		...
		<s:iterator id="result" value="#session.reportList" status="index">
			<tr class="startSortable">
				...
				<td nowrap="nowrap">
					<s:a
						href="bo_rep_drawSaleReport_SearchExpand.action?reportType=Agent Wise Expand&start_date=%{#startDate}&end_Date=%{#endDate}&agentOrgId=%{gameNo}&stateCode=%{#state}&cityCode=%{#city}"
