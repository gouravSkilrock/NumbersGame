function exportToExcel(tableId) {
	...
	$('#' + tableId + ' > thead:first >tr').each(function(index) {
		...
		row.find('th').each(function(colno) {
			...
			var isRowIncrement = true;
			if (rowSpan > 1) {
				var rowFilled =  Math.floor(rowSpan/2);
				for ( var i = index; i <= rowSpan; i++) {
					if (isRowIncrement) {
			...
			var isColIncrement = true;
			if (colSpan > 1) {
				var coloumnFilled =  Math.floor(colSpan/2);
				for ( var j = 0; j < colSpan; j++) {
					if (isColIncrement) {
						rowArr[index][coloumnFilled+currentCol].isMergedCell = 'YES';
						var colLast = currentCol + parseInt(colSpan) - 1;
						rowArr[index][coloumnFilled+currentCol].mergeRange = index + "," + index + "," + coloumnFilled+currentCol + "," + colLast;
						rowArr[index][coloumnFilled+currentCol].isDataSet = 'YES';
						rowArr[index][coloumnFilled+currentCol].data = colText;
					} else {
						currentCol++;
						rowArr[index][currentCol].isDataSet = 'YES';
					}
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/js/Export_Excel_Safaribet.js,v $'] = '$Id: Export_Excel_Safaribet.js,v 1.3 2016/10/31 09:45:31 neeraj Exp $';