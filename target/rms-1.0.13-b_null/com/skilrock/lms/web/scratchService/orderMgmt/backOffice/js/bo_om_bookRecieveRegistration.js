...
function verifyWareMaster()
{
	var e = document.getElementById("wareOrgName");
	var orgDisplayName = e.options[e.selectedIndex].text;
  return verifyOrg('wareOrgName',orgDisplayName,'You have selected Warehouse Master:',e.value);
}
...
function getRetailerList() {
		...
		var text = _ajaxCall("getWarehouseMasterNameList.action");
		var firstArr = text.data.split(":");
		//alert(firstArr);
		var retObj = _id.o("wareOrgName");
		document.getElementById("wareOrgName").options.length = 1;
...
_id.hid('subTrans');_id.hid('verBk');
if(validateData()){
url = "bo_om_bookRecieveRegistration_VerifyBooks.action?wareOrgName="+_id.v('wareOrgName')+"&"+countGmBks();
...
var count=0,gmNoLnth=2;gmInSelBx=0;

function appendGameTab(){
var numOfGame = _id.v('nog');gmInSelBx = 3;

if(numOfGame>gmInSelBx){
gmNoLnth = gmInSelBx;
document.getElementsByName('addGame')[0].style.visibility="hidden";
if(gmNoLnth<1)
_id.i('top_form','<h3>'+document.getElementsByTagName('h3')[0].innerHTML+'</h3><br/><center>No Game Available</center>');
}
else{
gmNoLnth = numOfGame;
}
...
function fetchDet(){
	crDiv();
	_id.non('top_form');
	var retSelBox = _id.o('wareOrgName'),gameSelBox=_id.o('gameNbr_NameHid');
	var _resp  = _ajaxCall("getGameNameWarehouse.action");
	if(_resp.result){
	_respData=_resp.data;
...
function frmValidtr(){
	var frmVal  = new parent.frames[0].Validator("bookRecieveRegistration");
    frmVal.aV("wareOrgName","dontselect=0","Please select a valid Warehouse Master","retOrgDiv");
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/orderMgmt/backOffice/js/bo_om_bookRecieveRegistration.js,v $'] = '$Id: bo_om_bookRecieveRegistration.js,v 1.3 2016/10/31 09:47:05 neeraj Exp $';
