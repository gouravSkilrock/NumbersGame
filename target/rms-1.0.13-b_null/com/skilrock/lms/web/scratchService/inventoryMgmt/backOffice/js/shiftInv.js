...
$("#retOrgDiv").html('');
if(validateData()){
url = "bo_im_invWarehouseShift_verifyInv.action?fromWarehouse="+_id.v('fromWH')+"&"+"toWarehouse="+_id.v('toWH')+"&"+countGmBks();
var _resp  = _ajaxCall(url);
	...
	if(_respData == 'success'){
		...
		for(var i = 0; i < gameSelected; i++)
		{
			var from = $("#0bookNoFrom_"+i).val().replace('-','');
			var to = $("#0bookNoTo_"+i).val().replace('-','');
			var display = 0;
			...
			if(display > 0){
				$("#0bookNoTo_"+i).after(function() {
					  return "<td><div id='errorDiv'><font color='red'>This Series is completely not available.</font></div></td>";
				});
			}
				
		}
		
		$('#top_form').find("input[type=\'text\']").each(function (index) {
			if(($(this).val().trim() != '' || $(this).val() != null ) && ($(this).attr('id').indexOf("From") < 0 && $(this).attr('id').indexOf("To") < 0) ){
			var textId = $(this).attr('id');
			
			
			for(i = 0; i < arr.length-1; i++){
				//alert(textId.indexOf('From') + " "+ textId);
				if(($(this).val().trim()) == arr[i].trim()){
					$('#'+textId).after(function() {
						  return "<td><div id='errorDiv'><font color='red'>Book doesn't belong to the selected warehouse.</font></div></td>";
					});
				}
			}}
		});
...
function submitInv(){
	url = "bo_im_invWarehouseShift_save.action?fromWarehouse="+_id.v('fromWH')+"&"+"toWarehouse="+_id.v('toWH')+"&"+countGmBks();
	var _resp  = _ajaxCall(url);
	_respData=_resp.data;
	var data = _respData.split(",");
	window.location = "backOffice/bo_im_warehouseShiftInv_Success.jsp?dcId="+data[0] ;
	return false;
	}
...
var _resp  = _ajaxCall("fetchChildOrgNGamenFmtSale.action?ownerType=BO&shiftGameStatus=ALL");
	if(_resp.result){
	_respData=_resp.data;
...
function validateData(){
		var chkD = checkDupBk();hidSubmit();
	  	if(validateWarehouse()&&_un()&&(chkD.All&&!chkD.Emp)){
	  	 return true;
	  	}
	  	if(chkD.Emp){
...
function validateWarehouse(){
	var fromWarehouse = _id.o("fromWH").value ;
	var toWarehouse = _id.o("toWH").value ;
	
	if(fromWarehouse == '-1'){
		$("#retOrgDiv").html("<font color = 'red'>Please Select Source Warehouse</font>");
		return false ;
	}
	if(toWarehouse == '-1'){
		$("#retOrgDiv").html("<font color = 'red'>Please Select Destination Warehouse</font>");
		return false ;
	}
	if(fromWarehouse == toWarehouse){
		$("#retOrgDiv").html("<font color = 'red'>Intra-Warehouse shifting is invalid. Please change either source or destination</font>");
		return false ;
	}
	return true ;
}
...
if(gmDigIn!=null && gmDigIn.value!=-1){
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/inventoryMgmt/backOffice/js/shiftInv.js,v $'] = '$Id: shiftInv.js,v 1.3 2016/10/31 09:46:19 neeraj Exp $';
