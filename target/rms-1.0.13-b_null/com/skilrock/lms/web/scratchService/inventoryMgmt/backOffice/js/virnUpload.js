...
function validateDates() {
	...
	if (document.getElementById("game_Name").value == "Please Select") {
		isInValid = true;
		document.getElementById("gamename1").innerHTML = "<font color = 'red'>"+i18nObj.prop('error.select.game.name')+"</font>";
	...
	if (startDate == "" || saleEndDate == "" || pwtEndDate == "") {
       ...
       // return false;
		...
		if (saleEndDate <= startDate) {
			isInValid = true;
			document.getElementById("dates").innerHTML = "<font color = 'red'>"+i18nObj.prop('error.sale.enddate.gt.strtdate')+"</font>";
          ...
          //alert("sale end date should be greater then start date");
			...
			if (saleEndDate >= pwtEndDate) {
				isInValid = true;
				document.getElementById("dates").innerHTML = "<font color = 'red'>"+i18nObj.prop('error.pwt.enddate.gt.sale.enddate')+"</font>";
	...
	if (document.getElementById("file").value == "") {
		isInValid = true;
		document.getElementById("file1").innerHTML = "<font color = 'red'>"+i18nObj.prop('error.pls.slct.virn.file')+"</font>";
         //alert('please select game name');
	} else {
...
function filevaliditycheck(gamename, filepath) {
	...
	if(_resp.result){
			...
			if (res.match("false")) {
				...
				document.getElementById("file1").innerHTML = "<font color = 'red'>"+i18nObj.prop('error.file.uploaded.previously')+"</font>";
...
parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/inventoryMgmt/backOffice/js/virnUpload.js,v $'] = '$Id: virnUpload.js,v 1.3 2016/10/31 09:46:19 neeraj Exp $';