		...
		function verifyNSave(){
			...
			if(tktNo==""){
				_id.i("tktNo_e", i18nObj.prop('errror.ticket.no.empty'), "e");
				isValid = false;
			}else {
			...
			if(virnCode==""){
				_id.i("virnNbr_e", i18nObj.prop('errror.virin.code.empty'), "e");
				isValid = false;
			}
			else if((gameName!=-1 && virnCode.length !=(gameDetMap[gameName])['game_virn_digits'])){
				_id.i("virnNbr_e", i18nObj.prop('error.virn.code.not.valid.format'), "e");
				isValid = false;
			}else{
				_id.i("virnNbr_e", "");
			}
		...
		parent.frames[0].Version['$Source: /rep/LMS_Mgmt/WebRoot/com/skilrock/lms/web/scratchService/pwtMgmt/backOffice/js/gameTktVirnvalidate.js,v $'] = '$Id: gameTktVirnvalidate.js,v 1.3 2016/10/31 09:46:58 neeraj Exp $';