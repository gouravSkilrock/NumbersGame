...
@PrepareForTest({DrawGameValidation.class, Util.class, orgOnLineSaleCreditUpdation.class, IDBarcode.class, CommonMethods.class, ResponsibleGaming.class,DBConnect.class})
...
public class DrawGameRPOSHelperTest {
	...
	public void zimLottoBonusPurchaseTicket_SetGameMap() {
		...
		Util.orgDataMap = new HashMap<Integer, Map<Integer,OrgDataBean>>();
		Map<Integer, List<PromoGameBean>> promoMap = new HashMap<Integer, List<PromoGameBean>>();
		List<PromoGameBean> promoList = new ArrayList<PromoGameBean>();
		promoMap.put(5,promoList);
		Util.promoGameBeanMap=promoMap ;
		Util.advMsgDataMap = new HashMap<Integer, Map<Integer,List<MessageDetailsBean>>>();
	...
	private IServiceDelegate zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate(final String isStatusSuccess) {
		 IServiceDelegate iServiceDelegate = new IServiceDelegate() {
			...
			public ServiceResponse getResponse(ServiceRequest sReq) {
				...
				if ("SUCCESS".equalsIgnoreCase(isStatusSuccess)) {
					response.setIsSuccess(true);
				}
				else if (isStatusSuccess == null || "FAILED".equalsIgnoreCase(isStatusSuccess)) {
	...
	private void zimLottoBonusPurchaseTicket_PowerMockStaticMethods() throws Exception {
		...
		ajaxHelper=Mockito.mock(AjaxRequestHelper.class);
		...
		PowerMockito.when(Util.getUnitPrice(Mockito.anyInt(), Mockito.anyString())).thenReturn(0.2);
		PowerMockito.mockStatic(CommonMethods.class);
		PowerMockito.when(CommonMethods.fmtToTwoDecimal(Mockito.anyDouble())).thenReturn(4.0);
		
		PowerMockito.when(Util.class,"getDGSaleAdvMessage",Mockito.anyInt(),Mockito.anyInt()).thenReturn(null);		
	}

	private void zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(boolean isFraud, long balanceDeduction, int ticketUpdate)
			throws Exception {
		PowerMockito.mockStatic(ResponsibleGaming.class);
		PowerMockito
				.when(ResponsibleGaming.respGaming(Mockito.any(UserInfoBean.class), Mockito.anyString(), Mockito.anyString(), Mockito.any(Connection.class)))
				.thenReturn(isFraud);
		PowerMockito.mockStatic(orgOnLineSaleCreditUpdation.class);
		PowerMockito.when(orgOnLineSaleCreditUpdation.drawTcketSaleBalDeduction(Mockito.any(UserInfoBean.class), 
						Mockito.anyInt(), Mockito.anyDouble(), Mockito.anyString(), Mockito.any(Connection.class)))
				.thenReturn(balanceDeduction);;
		PowerMockito.when(orgOnLineSaleCreditUpdation.drawTcketSaleBalDedUpdate(Mockito.any(UserInfoBean.class), 
						Mockito.anyInt(), Mockito.anyDouble(),Mockito.anyDouble(), Mockito.anyLong(), Mockito.any(Connection.class)))
				.thenReturn(balanceDeduction);
		PowerMockito.when(orgOnLineSaleCreditUpdation.class, "drawTicketSaleTicketUpdate", Mockito.anyLong(),
				...
				Mockito.any(Connection.class)).thenReturn(ticketUpdate);
	}

	private void zimLottoBonusPurchaseTicket_PowerMockValidationMethod(boolean isValid) {
		PowerMockito.mockStatic(DrawGameValidation.class);
		PowerMockito.when(DrawGameValidation.zimLottoBonusDataValidation(Mockito.any(LottoPurchaseBean.class))).thenReturn(isValid);
	}
	...
	public void zimLottoBonusPurchaseTicket_NoOfLinesIsNotZero() throws Exception {
		...
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, 13204548L, 1);
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("SUCCESS");
	...
	public void zimLottoBonusPurchaseTicket_SaleStatusFailedIfNumberPickedIsGreaterThanFifteen() throws Exception {
		...
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(false);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, 13204548L, 1);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("SUCCESS");
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
	...
	@Test
	public void zimLottoBonusPurchaseTicket_AgentHasInsufficientBalance() throws Exception {
		zimLottoBonusPurchaseTicket_SetUpDrawPurchaseBeanForPermSix();
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, -1L, 1);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("SUCCESS");
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
				drawPurchaseBean);
		Assert.assertEquals("Sale status is Success", "AGT_INS_BAL", zimLottoBonusPurchaseBean.getSaleStatus());
	}
	@Test
	public void zimLottoBonusPurchaseTicket_RetailerHasInsufficientBalance() throws Exception {
		zimLottoBonusPurchaseTicket_SetUpDrawPurchaseBeanForPermSix();
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, 0, 1);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("SUCCESS");
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
				drawPurchaseBean);
		Assert.assertEquals("Sale status is Success", "RET_INS_BAL", zimLottoBonusPurchaseBean.getSaleStatus());
	}
	@Test
	public void zimLottoBonusPurchaseTicket_StatusFailedBecauseLMSError() throws Exception {
		zimLottoBonusPurchaseTicket_SetUpDrawPurchaseBeanForPermSix();
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, -2L, 1);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("SUCCESS");
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
				drawPurchaseBean);
	...
	@Test
	public void zimLottoBonusPurchaseTicket_StatusIsFraud() throws Exception {
		zimLottoBonusPurchaseTicket_SetUpDrawPurchaseBeanForPermSix();
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(true, 13204548L, 1);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("SUCCESS");
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
				drawPurchaseBean);
		Assert.assertEquals("Sale status is Success", "FRAUD", zimLottoBonusPurchaseBean.getSaleStatus());
	}
	@Test
	public void zimLottoBonusPurchaseTicket_TicketNotUpdatedOnDGE() throws Exception {
		zimLottoBonusPurchaseTicket_SetUpDrawPurchaseBeanForPermSix();
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, 13204548L, 0);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("SUCCESS");
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
				drawPurchaseBean);
	...
	@Test
	public void zimLottoBonusPurchaseTicket_SaleStatusIsNull() throws Exception {
		zimLottoBonusPurchaseTicket_SetUpDrawPurchaseBeanForPermSix();
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, 13204548L, 1);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate(null);
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
				drawPurchaseBean);
		Assert.assertEquals("Sale status is Success", "FAILED", zimLottoBonusPurchaseBean.getSaleStatus());
	}
	@Test
	public void zimLottoBonusPurchaseTicket_ServiceResponseIsFailed() throws Exception {
		zimLottoBonusPurchaseTicket_SetUpDrawPurchaseBeanForPermSix();
		zimLottoBonusPurchaseTicket_PowerMockStaticMethods();
		zimLottoBonusPurchaseTicket_PowerMockValidationMethod(true);
		zimLottoBonusPurchaseTicket_PowerMockRespGamingBalanceDeductionAndTicketUpdateMethods(false, 13204548L, 1);
		AjaxRequestHelper ajaxHelper = new AjaxRequestHelper();
		IServiceDelegate iServiceDelegate = zimLottoBonusPurchaseTicket_InnerClassIServiceDelegate("FAILED");
		LottoPurchaseBean zimLottoBonusPurchaseBean = new DrawGameRPOSHelper(iServiceDelegate,ajaxHelper).zimLottoBonusPurchaseTicket(userBean,
				drawPurchaseBean);
		Assert.assertEquals("Sale status is Success", "FAILED", zimLottoBonusPurchaseBean.getSaleStatus());
	}
