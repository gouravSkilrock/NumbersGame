...
public class CollectionLiveReportOverAllHelper implements ICollectionLiveReportOverAllHelper{
	Log logger = LogFactory.getLog(CollectionLiveReportOverAllHelper.class);
	...
	public Map<String, String> allGameMap() throws LMSException {
		...
		ResultSet rs = null;
		try {
			String gameQry = "select game_name,'DG' as game_type from st_dg_game_master union all select game_name,'SE' as game_type from st_se_game_master union all select category_code,'CS' as game_type from st_cs_product_category_master union all select wallet_name, 'OLA' as game_type from st_ola_wallet_master where wallet_status = 'ACTIVE' order by game_type";
	...
	public void collectionAgentWise(Timestamp startDate, Timestamp endDate,
			Connection con, boolean isDG, boolean isSE, boolean isCS, boolean isOLA,
	...
	public void collectionAgentWiseExpand(Timestamp startDate,
			Timestamp endDate, boolean isDG, boolean isSE, boolean isCS, boolean isOLA, boolean isSLE,
	...
	public Map<String, CollectionReportOverAllBean> collectionAgentWiseWithOpeningBal(
			...
			boolean isDG, boolean isSE, boolean isCS, boolean isOLA, boolean isSLE, ReportStatusBean reportStatusBean) throws LMSException {
		PreparedStatement pstmt = null;
		...
		try {
			...
			String agtOrgQry = "select "+orgCodeQry+",organization_id from st_lms_organization_master where organization_type='AGENT' order by "+QueryManager.getAppendOrgOrder();
			...
			collectionAgentWise(deployDate, startDate, con, isDG, isSE, isCS, isOLA,
					agtMap);
			...
			collectionAgentWise(startDate, endDate, con, isDG, isSE, isCS, isOLA,
					agtMap);
		} catch (Exception e) {
			e.printStackTrace();
