...
public class GenerateDeliveryForAgentByHtmlHelper {
	...
	public Map getInvoiceDetailNew(int id) {
		con = DBConnect.getConnection();
		String query = QueryManager.getST_AGENT_INVOICE_DETAILS();
		try {
			PreparedStatement pstmt = con.prepareStatement(query);
			...
			while (rs.next()) {
				if (flag) {
					...
					packNbrSet.add(rs.getString("pack_nbr"));
					bookNbrList.add(rs.getString("total_books"));
					transactionDate = new SimpleDateFormat("dd/MM/yyyy").format(sqlDateToutilDate(rs.getDate("transaction_date")));
					getInvoiceForCustomerDetail(rs.getInt("retailer_org_id"), con);
					if ("DSRCHALLAN".equalsIgnoreCase(this.type.trim()))
						getInvoiceIDFORSaleRet(id, con);
					else
				...
				if (gameIdSet.contains(gameId)) {
					...
					int booksPerPack = rs.getInt("books_per_pack");
					gameDetailBean.setNbrBooks(booksPerPack);
					if (packNbrSet.contains(packNbr) && bookNbrList.size() < booksPerPack) {
						bookNbrList.add(bookNbr);
						if (bookNbrList.size() == booksPerPack) {
							gameDetailBean.setPackNbrList(packNbr);
							bookNbrList.clear();
	...
	@SuppressWarnings("unchecked")
	private void getInvoiceIDFORSaleNew(int id, Connection connection) throws SQLException, ParseException {
		if (connection == null)
			connection = DBConnect.getConnection();

		PreparedStatement pstmt = connection.prepareStatement("SELECT generated_id, ao.order_id, order_date FROM st_lms_agent_receipts ar INNER JOIN st_se_agent_order_invoices aoi ON ar.receipt_id=aoi.dc_id INNER JOIN st_se_agent_order ao ON aoi.order_id=ao.order_id WHERE dc_id=?;");
		pstmt.setInt(1, id);
		ResultSet rs = pstmt.executeQuery();
		if (rs != null) {
			if (rs.next()) {
				...
				detailMap.put("orderId", rs.getInt("order_id"));
				detailMap.put("orderDate", new SimpleDateFormat("dd/MM/yyyy").format(sqlDateToutilDate(rs.getDate("order_date"))));
			}
		}
	}
