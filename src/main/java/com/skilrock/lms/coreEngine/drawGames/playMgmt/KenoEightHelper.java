...
public class KenoEightHelper {

	Log logger =  LogFactory.getLog(KenoEightHelper.class);
	...
	public KenoPurchaseBean kenoPurchaseTicket(UserInfoBean userBean,
		...
		sReq.setServiceName(ServiceName.KENO_EIGHT_MGMT);
		sReq.setServiceMethod(ServiceMethodName.KENOEIGHT_PURCHASE_TICKET);
		...
		for (int i = 0; i < noOfPanel; i++) {
			...
			if (playType.contains("Direct")) {
				noOfLines[i] = 1;
			} else if (playType.equalsIgnoreCase("perm2") || playType.equalsIgnoreCase("DC-perm2") || playType.equalsIgnoreCase("MN-perm2")) {
		...
		/*	for (int i = 0; i < noOfPanel; i++) {
			...
			for (int i = 0; i < noOfPanel; i++) {
				...
				if (playTypeArr[i].contains("Direct")
					...
					int pickValue = KenoEightConstants.BET_TYPE_MAP.get(playTypeArr[i]);
				...
				/*	isValid = noPickedArr[i].equals(pickValue);
					...
					int minPickValue=KenoEightConstants.BET_TYPE_MAP.get(playTypeArr[i]+"MIN");
					int maxPickValue=KenoEightConstants.BET_TYPE_MAP.get(playTypeArr[i]+"MAX");
					...
					int minULPickValue=KenoEightConstants.BET_TYPE_MAP.get(playTypeArr[i]+"ULMIN");
					int maxULPickValue=KenoEightConstants.BET_TYPE_MAP.get(playTypeArr[i]+"ULMAX");
					int minBLPickValue=KenoEightConstants.BET_TYPE_MAP.get(playTypeArr[i]+"BLMIN");
					int maxBLPickValue=KenoEightConstants.BET_TYPE_MAP.get(playTypeArr[i]+"BLMAX");
					...
					if (qp[i]!=2 || minULPickValue >pickedUL
						...
						break;
							...
							isValid = Util
							.validateNumber(KenoEightConstants.START_RANGE,
									KenoEightConstants.END_RANGE, playerData.replace(
											",UL,", ",").replace(",BL", ""), false);
