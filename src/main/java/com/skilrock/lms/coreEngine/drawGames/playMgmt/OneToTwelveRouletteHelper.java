...
public class OneToTwelveRouletteHelper {

	Log logger = LogFactory.getLog(OneToTwelveRouletteHelper.class);
	...
	public OneToTwelveRoulettePurchaseBean oneToTwelveRoulettePurchaseTicket(UserInfoBean userBean, OneToTwelveRoulettePurchaseBean purchaseBean) throws LMSException {
		...
		sReq.setServiceName(ServiceName.ROULETTE_MGMT);
		sReq.setServiceMethod(ServiceMethodName.ROULETTE_PURCHASE_TICKET);
		OneToTwelveRoulettePurchaseBean requestBean = new OneToTwelveRoulettePurchaseBean();
		...
		try {
			...
			if (!oneToTwelveRouletteValidateData(purchaseBean)) {
			...
			for (int i = 0; i < noOfPanel; i++) {
				...
				noOfLines[i] = noPick.length;*/
				/*unitPriceArr[i] = Util.getUnitPrice(purchaseBean.getGameId(), playTypeArr[i]);*/
				totPurAmt += purchaseBean.getUnitPrice()[i] * purchaseBean.getNoOfDraws();
			}
...
*/
		...
		try {
			...
			requestBean.setIsQP(purchaseBean.getIsQP());
			...
			requestBean.setTotalNoOfPanels(purchaseBean.getTotalNoOfPanels());
			requestBean.setNoOfPanel(purchaseBean.getNoOfPanel());
			requestBean.setNoOfLines(purchaseBean.getNoOfLines());
			...
			if (responseBean.getIsSuccess()) {
				...
				purchaseBean.setPlayerData(responseBean.getPickedNumbers());
				...
				purchaseBean.setNoOfLines(responseBean.getNoOfLines());
	...
	public boolean oneToTwelveRouletteValidateData(OneToTwelveRoulettePurchaseBean purchaseBean) throws IllegalArgumentException, SecurityException, IllegalAccessException, NoSuchFieldException {
		if (purchaseBean != null) {
			...
			String gameName = purchaseBean.getGameDevName().toUpperCase();
			for (int i = 0; i < noOfPanel; i++) {
			...
			if (!isValid) {
				...
				logger.info("-----------One To Twelve Roulette Validation Error-------------------" + purchaseBean.getSaleStatus());
				return false;
			}
		} else {
			logger.debug("One To Twelve Roulette Data Error : Null Bean Recieved ");
			return false;
		}
		return true;
	}
