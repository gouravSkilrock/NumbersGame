...
public class ZimLottoAction extends ActionSupport implements
	...
	static Log logger = LogFactory.getLog(ZimLottoAction.class);
	/**
	 * 
	 */
	...
	public void purchaseTicketProcess() throws Exception {
		...
		if (picknumbers.length < 1) {
			...
			System.out.println("FINAL PURCHASE DATA ZIMLOTTO:" + finalPurchaseData);
		...
		for (int i = 0; i < picknumbers.length; i++) {
			if (!"QP".equals(picknumbers[i])) {
				if (!Util.validateNumber(
					...
					finalPurchaseData = "ErrorMsg:"
							+ EmbeddedErrors.PURCHSE_INVALID_DATA;
					System.out.println("FINAL PURCHASE DATA ZIMLOTTO:"
							+ finalPurchaseData);
					response.getOutputStream().write(
		...
		finalPurchaseData = "TicketNo:" + lottoPurchaseBean.getTicket_no()
				+ lottoPurchaseBean.getReprintCount() +((ConfigurationVariables.currentTktLen == ConfigurationVariables.tktLenA)?"":lottoPurchaseBean.getBarcodeCount())+ "|Date:" + time + ""
				+ stBuilder.toString() + "|TicketCost:"
				+ lottoPurchaseBean.getTotalPurchaseAmt()
				+ drawTimeBuild.toString() + "|balance:" + balance + "|QP:"
				+ lottoPurchaseBean.getIsQuickPick()[0] + "|" + raffleData
				+ advtMsg;
