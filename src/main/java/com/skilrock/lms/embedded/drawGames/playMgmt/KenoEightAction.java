...
import com.skilrock.lms.coreEngine.drawGames.playMgmt.KenoEightHelper;
...
public class KenoEightAction extends ActionSupport implements ServletRequestAware,
		ServletResponseAware {

	static Log logger = LogFactory.getLog(KenoEightAction.class);
	/**
	 * 
	 */
	...
	private int gameId = Util.getGameId("KenoEight");
	...
	public void purchaseTicketProcess() {
		...
		KenoEightHelper helper = new KenoEightHelper();
		...
		logger.debug("SALE_AUTO_CANCEL_LOGS:" + "SALE Continue for the new ticket");*/
         ...
         if("NIGERIA".equals(countryDeployed)){
 	      ...
 	      String bettypeOffStartTime=(String) sc
	        .getAttribute("BETTYPE_OFF_START_TIME");
 	    
 	     // logger.info("BETTYPE_OFF_START_TIME::"+bettypeOffStartTime);
 	     Date start = parser.parse(bettypeOffStartTime);
 	      
 	      String bettypeOffEndTime=(String) sc
	        .getAttribute("BETTYPE_OFF_END_TIME");
 ...
 /*	        String currTime=(calendar.get(Calendar.HOUR_OF_DAY)+":"+calendar.get(Calendar.MINUTE)+":"+calendar.get(Calendar.SECOND));
 	        logger.info("current time::"+currTime);
 	        if(currTime.compareTo(bettypeOffStartTime)>0){
 	            if(currTime.compareTo(bettypeOffEndTime)<0){
 	                try{     
 	                    for(String betType:playTypeArr){
 	                    if("Direct4".equals(betType) || "Direct5".equals(betType)){
 	                        response.getOutputStream().write(("ErrorMsg:Server Busy Amount Not Deducted").getBytes());
		...
		StringBuilder drawTimeBuild = new StringBuilder("");
			...
			for (int i = 0; i < listSize; i++) {
				//drawTimeBuild.append(("|DrawDate:" + Util.changeFormat("dd-MM-yyyy HH:mm:ss", "yyyy-MM-dd HH:mm:ss", kenoPurchaseBean.getDrawDateTime().get(i))).replaceFirst(" ", "#").replace("#", "|DrawTime:").replace("&", "|DrawId:").replace(".0", ""));
...
*/
        ...
        if(!"".equals(raffleGameDataString)){
            ...
            for(int i=0;i < raffGameArray.length;i++){
                ...
                if("Keno".equalsIgnoreCase(raffleGameData.substring(0,raffleGameData.indexOf("%")))){
		...
		response.getOutputStream().write(finalPurchaseData.getBytes());
			...
			System.out.println("FINAL PURCHASE DATA KENO:Error!Try Again");
			try {
