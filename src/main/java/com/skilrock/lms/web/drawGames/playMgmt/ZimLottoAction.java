...
public class ZimLottoAction extends ActionSupport implements
	...
	private int gameNumber = Util.getGameId("Zimlotto");
	private int isAdvancedPlay;
	Log logger = LogFactory.getLog(ZimLottoAction.class);
	...
	public String purchaseTicketProcess() throws Exception {
		...
		/*
		 * int noOfPanels = picknumbers.length; int[] betAmountMultiples = new
		 * int[noOfPanels]; int[] panel_id=new int[noOfPanels]; Integer
		 * isQuickPick[]= new Integer[noOfPanels];
		 * 
		 * for (int i = 0; i < noOfPanels; i++) { if
		 * (picknumbers[i].equalsIgnoreCase("QP")) {
		 * playerPicked.add(Util.getRandomNoKeno(startRange,endRange,winNumbers));
		 * isQuickPick[i]=1; } else { playerPicked.add(picknumbers[i]);
		 * isQuickPick[i]=2; } betAmountMultiples[i]=1; panel_id[i]=i+1; }
		 */
		...
		setLottoPurchaseBean(helper.zimLottoPurchaseTicket(userBean,
				lottoPurchaseBean));
