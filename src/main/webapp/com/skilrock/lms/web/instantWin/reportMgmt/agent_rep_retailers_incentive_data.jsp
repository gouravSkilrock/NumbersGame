	...
	<div align="center">
		<s:form action="bo_rep_retailers_Incentive_ExpExcel" method="post"
			...
			<s:submit value="Export As Excel" align="center"
				onclick="getRetailerData();" />
		</s:form>
	</div>
</s:if>
<div id="tableDataDivForRetailer">
	 <s:if test="%{reportType.equals('Weekly')}">
		<div align="center">
			<h2>
	...
	<table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC" align="center" style="font-size: 12px">
		<s:if test="retailerMap != null && retailerMap.size() > 0">
			...
			<tr>
				...
				<th></th>
				<th>
					<%=FormatNumber.formatNumberForJSP(pageContext.getAttribute("saleSum"))%>
				</th>
				<th>
					<%=FormatNumber.formatNumberForJSP(pageContext.getAttribute("winSum"))%>
				</th>
				<th>
					<%=FormatNumber.formatNumberForJSP(pageContext.getAttribute("nonWinSum"))%>
				</th>
				<th>
					<%=FormatNumber.formatNumberForJSP(pageContext.getAttribute("incentiveSum"))%>
				</th>
			</tr>
		</s:if>
		<s:else>
...
<code id="headId" style="visibility: hidden">
	$RCSfile: agent_rep_retailers_incentive_data.jsp,v $ $Revision: 1.3 $ </code>