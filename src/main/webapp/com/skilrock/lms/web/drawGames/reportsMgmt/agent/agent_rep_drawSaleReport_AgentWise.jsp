...
<s:if test="#session.reportList.size>0">
				<s:div id="excel"
					...
					<s:a href="agent_rep_drawSaleReport_exportExcel.action?reportType=Retailer Wise&start_date=%{#startDate}&end_Date=%{#endDate}&agentOrgId=%{#agtOrgId}&stateCode=%{#state}&cityCode=%{#city}"><s:text name="btn.exportasexcel"/></s:a>
				</s:div>
</s:if>
	...
	<s:if
		...
		<s:iterator id="result" value="#session.reportList" status="index">
			<tr class="startSortable">
				...
				<td nowrap="nowrap">
					<s:a
						href="agent_rep_drawSaleReport_SearchExpand.action?reportType=Retailer Wise Expand&start_date=%{#startDate}&end_Date=%{#endDate}&agentOrgId=%{gameNo}&stateCode=%{#state}&cityCode=%{#city}"
