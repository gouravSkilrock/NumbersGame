...
<body>
	...
	<table width="100%" border="1" cellpadding="3" cellspacing="0"
		...
		<tr>
			<td colspan="11" align="center" nowrap="nowrap"><b><s:text
						name="btn.srch.res" /> </b>
			</td>
		</tr>
		...
		<s:if test=" #session.ORG_SEARCH_RESULTS1.size !=0 ">
			<tr>
				<th align="center"><s:text name="label.org.name" />
				</th>
				<th align="center"><s:text name="label.type" />
				</th>
				<th align="center"><s:text name="label.status" />
				</th>
				<th align="center"><s:text name="label.prnt.org" />
				</th>
				<th align="center"><s:text name="label.pwt.scrp" />
				</th>
				<s:if test="orgType == 'RETAILER'">
					<th align="center"><s:text name="label.terminal.status"></s:text>
					</th>
				</s:if>
				<th align="center"><s:text name="CL" />
				</th>
				<th align="center"><s:text name="XCL" />
				</th>
				<th align="center"><s:text name="label.rem.day" />
				</th>
				<s:if test="#application.COUNTRY_DEPLOYED == 'KENYA'">
					<th align="center"><s:text name="label.ledger.bal" />
					</th>
				</s:if>
				<th align="center"><s:text name="label.blnce" />
				</th>
				<th align="center"><s:text name="label.claim.bal" />
				</th>
				<th align="center"><s:text name="label.secur.dpst" />
				</th>
			</tr>
			<s:iterator id="SearchOrgResults" value="#session.ORG_SEARCH_RESULTS"
				...
				<tr id="<s:property value="#rowIndex.index"/>">
					...
					%>
					<td><s:a id="link2" targets="top"
							href="bo_um_extendCrLimit_Detail.action?orgId=%{orgId}">
							<s:property value="orgName" />
						</s:a></td>
					<td><s:property id="gn"
							value="#application.TIER_MAP[orgType].toUpperCase()" />
					</td>
					<td><s:property value="orgStatus" />
					</td>
					<td><s:property value="parentOrgName" />
					</td>
					<td><s:property value="pwtScrapStatus" />
					</td>
					...
					<td align="right" nowrap="nowrap"><s:property
							value="extendsCreditLimitUpto" />
					</td>
					<s:if test="#application.COUNTRY_DEPLOYED == 'KENYA'">
						<s:if test="%{ledgerBalance<0}">
							<td align="right" nowrap="nowrap" style="color: red"><s:a
									href="#"
						...
						<s:else>
							<td align="right" nowrap="nowrap"><s:a href="#"
									onclick="updatedLedgerBalance('com/skilrock/lms/admin/bo_um_searchAgtRet_UpdLedgerBal.action?orgId=%{orgId}& parentOrgId=%{parentOrgId}& orgType=%{orgType}','div-%{trId}')">
									<s:div id="div-%{trId}"><%=strledgerBalance%></s:div>
									<div id="load-<s:property value="#rowIndex.index"/>"
										style="text-align: right"></div>
		...
		<s:else>
			<tr>
				<td colspan="11" align="center"><s:text
		...
		<s:if test=" #session.ORG_SEARCH_RESULTS1.size >5 ">
			...
			<s:if test="#session.startValueOrgSearch!=0">
				<s:a theme="ajax" targets="bottom"
					href="bo_um_extendCrLimit_Navigate.action?end=first">
					<s:text name="label.first" />
				</s:a>
				<s:a theme="ajax" targets="bottom"
					href="bo_um_extendCrLimit_Navigate.action?end=Previous">
			...
			<s:else>
				<s:a theme="ajax" targets="bottom"
					href="bo_um_extendCrLimit_Navigate.action?end=Next">
					<s:text name="label.next" />
				</s:a>
				<s:a theme="ajax" targets="bottom"
					href="bo_um_extendCrLimit_Navigate.action?end=last">
...
<code id="headId" style="visibility: hidden"> $RCSfile:
	bo_um_extendCrLimit_SearchResult.jsp,v $ $Revision:
	1.1.2.1.4.1.2.4.10.2 $ </code>